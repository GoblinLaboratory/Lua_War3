...
local function _wait(...)
    ...
    while true do
        local t2 = os.time()
        if t2 - t1 >= waitTime then return t2 - t1 end
    end
end
_M._wait = _wait

function _M:new(name)
    assert(self ~= nil and type(self) == "table" and self == _M,
           "Invalid EventLib table (make sure you're using ':' not '.')")
    ...
    return setmetatable(s, {
        __index = self,
        __call = function(t, ...) return t:fire(...) end
    })
end
...
function _M:Connect(handler)
    assert(self ~= nil and type(self) == "table",
           "Invalid Event (make sure you're using ':' not '.')")
    assert(type(handler) == "function",
           "Invalid handler. Expected function got " .. type(handler))
    assert(self.handlers, "Invalid Event")

    table.insert(self.handlers, handler)
    local t = {}
    t.func = handler
    t.Disconnect = function() return self:Disconnect(handler) end
...
function _M:Disconnect(handler)
    assert(self ~= nil and type(self) == "table",
           "Invalid Event (make sure you're using ':' not '.')")
    assert(type(handler) == "function" or type(handler) == "nil" or
               type(handler) == "table",
           "Invalid handler. Expected function, table, or nil, got " ..
               type(handler))
...
if false then
    ...
    print("Couldn't disconnect an already disconnected handler?",
          e2:disconnect() == nil)
