...
function BJ.CreateItemLoc (   itemId,   loc )
	bj_lastCreatedItem = CJ.CreateItem(itemId, CJ.GetLocationX(loc), CJ.GetLocationY(loc))
...
function BJ.CreateUnitAtLocSaveLast (   id,   unitid,   loc,   face )
    if (unitid == 'ugol') then
         bj_lastCreatedUnit = CJ.CreateBlightedGoldmine(id, CJ.GetLocationX(loc), CJ.GetLocationY(loc), face)
    else
...
function BJ.CreateNUnitsAtLocFacingLocBJ (   count,   unitId,   whichPlayer,   loc,   lookAt )
   return CJ.CreateNUnitsAtLoc(count, unitId, whichPlayer, loc, CJ.AngleBetweenPoints(loc, lookAt))
end

--===========================================================================
function BJ.AngleBetweenPoints (   locA,   locB )
    return bj_RADTODEG * CJ.Atan2(GetLocationY(locB) - CJ.GetLocationY(locA), CJ.GetLocationX(locB) - CJ.GetLocationX(locA))
end
...
function BJ.UnitAddItemByIdSwapped (   itemId,   whichHero )
    ...
    -- his inventory is full.
     bj_lastCreatedItem = CJ.CreateItem(itemId, CJ.GetUnitX(whichHero), CJ.GetUnitY(whichHero))
...
function BJ.TriggerRegisterPlayerSelectionEventBJ(trig,whichPlayer,selected)
    if selected then
        return CJ.TriggerRegisterPlayerUnitEvent(trig, whichPlayer, CJ.EVENT_PLAYER_UNIT_SELECTED, null)
    else
        return CJ.TriggerRegisterPlayerUnitEvent(trig, whichPlayer, CJ.EVENT_PLAYER_UNIT_DESELECTED, null)
    end
end
...
function BJ.QuestMessageBJ (   f,   messageType,   message )
    if (CJ.IsPlayerInForce(CJ.GetLocalPlayer(), f)) then
        ...
        if (messageType == bj_QUESTMESSAGE_DISCOVERED) then
            CJ.DisplayTimedTextToPlayer(CJ.GetLocalPlayer(), 0, 0, bj_TEXT_DELAY_QUEST, " ")
            CJ.DisplayTimedTextToPlayer(CJ.GetLocalPlayer(), 0, 0, bj_TEXT_DELAY_QUEST, message)
            CJ.StartSound(bj_questDiscoveredSound)
            CJ.FlashQuestDialogButton()

        elseif (messageType == bj_QUESTMESSAGE_UPDATED) then
            CJ.DisplayTimedTextToPlayer(CJ.GetLocalPlayer(), 0, 0, bj_TEXT_DELAY_QUESTUPDATE, " ")
            CJ.DisplayTimedTextToPlayer(CJ.GetLocalPlayer(), 0, 0, bj_TEXT_DELAY_QUESTUPDATE, message)
            CJ.StartSound(bj_questUpdatedSound)
            CJ.FlashQuestDialogButton()

        elseif (messageType == bj_QUESTMESSAGE_COMPLETED) then
            CJ.DisplayTimedTextToPlayer(CJ.GetLocalPlayer(), 0, 0, bj_TEXT_DELAY_QUESTDONE, " ")
            CJ.DisplayTimedTextToPlayer(CJ.GetLocalPlayer(), 0, 0, bj_TEXT_DELAY_QUESTDONE, message)
            CJ.StartSound(bj_questCompletedSound)
            CJ.FlashQuestDialogButton()

        elseif (messageType == bj_QUESTMESSAGE_FAILED) then
            CJ.DisplayTimedTextToPlayer(CJ.GetLocalPlayer(), 0, 0, bj_TEXT_DELAY_QUESTFAILED, " ")
            CJ.DisplayTimedTextToPlayer(CJ.GetLocalPlayer(), 0, 0, bj_TEXT_DELAY_QUESTFAILED, message)
            CJ.StartSound(bj_questFailedSound)
            CJ.FlashQuestDialogButton()

        elseif (messageType == bj_QUESTMESSAGE_REQUIREMENT) then
            CJ.DisplayTimedTextToPlayer(CJ.GetLocalPlayer(), 0, 0, bj_TEXT_DELAY_QUESTREQUIREMENT, message)

        elseif (messageType == bj_QUESTMESSAGE_MISSIONFAILED) then
            CJ.DisplayTimedTextToPlayer(CJ.GetLocalPlayer(), 0, 0, bj_TEXT_DELAY_MISSIONFAILED, " ")
            CJ.DisplayTimedTextToPlayer(CJ.GetLocalPlayer(), 0, 0, bj_TEXT_DELAY_MISSIONFAILED, message)
            CJ.StartSound(bj_questFailedSound)

        elseif (messageType == bj_QUESTMESSAGE_HINT) then
            CJ.DisplayTimedTextToPlayer(CJ.GetLocalPlayer(), 0, 0, bj_TEXT_DELAY_HINT, " ")
            CJ.DisplayTimedTextToPlayer(CJ.GetLocalPlayer(), 0, 0, bj_TEXT_DELAY_HINT, message)
            CJ.StartSound(bj_questHintSound)

        elseif (messageType == bj_QUESTMESSAGE_ALWAYSHINT) then
            CJ.DisplayTimedTextToPlayer(CJ.GetLocalPlayer(), 0, 0, bj_TEXT_DELAY_ALWAYSHINT, " ")
            CJ.DisplayTimedTextToPlayer(CJ.GetLocalPlayer(), 0, 0, bj_TEXT_DELAY_ALWAYSHINT, message)
            CJ.StartSound(bj_questHintSound)

        elseif (messageType == bj_QUESTMESSAGE_SECRET) then
            CJ.DisplayTimedTextToPlayer(CJ.GetLocalPlayer(), 0, 0, bj_TEXT_DELAY_SECRET, " ")
            CJ.DisplayTimedTextToPlayer(CJ.GetLocalPlayer(), 0, 0, bj_TEXT_DELAY_SECRET, message)
            CJ.StartSound(bj_questSecretSound)

        elseif (messageType == bj_QUESTMESSAGE_UNITACQUIRED) then
            CJ.DisplayTimedTextToPlayer(CJ.GetLocalPlayer(), 0, 0, bj_TEXT_DELAY_UNITACQUIRED, " ")
            CJ.DisplayTimedTextToPlayer(CJ.GetLocalPlayer(), 0, 0, bj_TEXT_DELAY_UNITACQUIRED, message)
            CJ.StartSound(bj_questHintSound)

        elseif (messageType == bj_QUESTMESSAGE_UNITAVAILABLE) then
            CJ.DisplayTimedTextToPlayer(CJ.GetLocalPlayer(), 0, 0, bj_TEXT_DELAY_UNITAVAILABLE, " ")
            CJ.DisplayTimedTextToPlayer(CJ.GetLocalPlayer(), 0, 0, bj_TEXT_DELAY_UNITAVAILABLE, message)
            CJ.StartSound(bj_questHintSound)

        elseif (messageType == bj_QUESTMESSAGE_ITEMACQUIRED) then
            CJ.DisplayTimedTextToPlayer(CJ.GetLocalPlayer(), 0, 0, bj_TEXT_DELAY_ITEMACQUIRED, " ")
            CJ.DisplayTimedTextToPlayer(CJ.GetLocalPlayer(), 0, 0, bj_TEXT_DELAY_ITEMACQUIRED, message)
            CJ.StartSound(bj_questItemAcquiredSound)

        elseif (messageType == bj_QUESTMESSAGE_WARNING) then
            CJ.DisplayTimedTextToPlayer(CJ.GetLocalPlayer(), 0, 0, bj_TEXT_DELAY_WARNING, " ")
            CJ.DisplayTimedTextToPlayer(CJ.GetLocalPlayer(), 0, 0, bj_TEXT_DELAY_WARNING, message)
...
function BJ.GetPlayableMapRect (   )
    ...
    return CJ.Rect(CJ.GetCameraBoundMinX()-CJ.GetCameraMargin(CAMERA_MARGIN_LEFT), CJ.GetCameraBoundMinY()-CJ.GetCameraMargin(CAMERA_MARGIN_BOTTOM), CJ.GetCameraBoundMaxX()+CJ.GetCameraMargin(CAMERA_MARGIN_RIGHT), CJ.GetCameraBoundMaxY()+CJ.GetCameraMargin(CAMERA_MARGIN_TOP))
end
...
BJ.ForGroupBJ  = function ( whichGroup , callback )
    ...
    BJ.RMinBJ  = function ( a , b )
        ...
        BJ.SinBJ  = function ( degrees )
        do return BJ.Sin ( degrees * bj_DEGTORAD ) end
        end
        
        BJ.CosBJ  = function ( degrees )
        do return BJ.Cos ( degrees * bj_DEGTORAD ) end
        end
        
        BJ.TanBJ  = function ( degrees )
        do return BJ.Tan ( degrees * bj_DEGTORAD ) end
        end
        
        BJ.AsinBJ  = function ( degrees )
        do return BJ.Asin ( degrees ) * bj_RADTODEG end
        end
        
        BJ.AcosBJ  = function ( degrees )
        do return BJ.Acos ( degrees ) * bj_RADTODEG end
        end
        
        BJ.AtanBJ  = function ( degrees )
        do return BJ.Atan ( degrees ) * bj_RADTODEG end
        end
        
        BJ.Atan2BJ  = function ( y , x )
        do return BJ.Atan2 ( y , x ) * bj_RADTODEG end
        end
    ...
    do return CJ.Location ( CJ.GetRectCenterX ( whichRect ) , CJ.GetRectCenterY ( whichRect ) ) end
    end
