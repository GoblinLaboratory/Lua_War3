...
function LifeAura:Int(u, jl)
    ...
    self.MoFaXiaoGuo = Global:GetId('B006')
     --，魔法效果
...
function LifeAura:AddUnit(u)
    local bu = Bonus:Instance()
    if self.JiNeng ~= 0 then
        CJ.UnitAddAbility(u, self.JiNeng)
     --添加技能
    end
    if self.Life ~= 0 then
...
function LifeAura:Group()
    ...
    if self.Ally then
        self.LinShiDanWeZu =
            ...
            self.JuLi,
            CJ.GetUnitLoc(self.TriggerUnit),
            CJ.Condition(
                function()
                    return CJ.IsUnitAlly(CJ.GetFilterUnit(), CJ.GetOwningPlayer(self.TriggerUnit)) == true and
        ...
        self.LinShiDanWeZu =
            ...
            CJ.Condition(
                function()
                    return CJ.IsUnitEnemy(CJ.GetFilterUnit(), CJ.GetOwningPlayer(self.TriggerUnit)) == true and
                        CJ.IsUnitType(x, CJ.UNIT_TYPE_STRUCTURE) == false
                end
            )
        )
    end
    ...
    CJ.GroupClear(self.LinShiDanWeZu)
 --清空单位组
end
