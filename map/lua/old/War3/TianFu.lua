...
local  function IntAbility(u,id)  
    local x = Global:GetStrId(CJ.GetUnitTypeId(GM.Maji_ID["玩家"..tostring(BJ.GetConvertedPlayerId(CJ.GetOwningPlayer(SU)))].Hero_ID))
    --local h = GM:GetHero(BJ.GetConvertedPlayerId(CJ.GetOwningPlayer(u)))
    local h = GM:GetHero(GM.Maji_ID["玩家"..tostring(BJ.GetConvertedPlayerId(CJ.GetOwningPlayer(u)))].Hero_ID)
...
CJ.TriggerAddAction(trig_TianFu,
    function () 
        ...
        local h = GM:GetHero(GM.Maji_ID["玩家"..tostring(BJ.GetConvertedPlayerId(CJ.GetOwningPlayer(SU)))].Hero_ID)
        local x = Global:GetStrId(CJ.GetUnitTypeId(GM.Maji_ID["玩家"..tostring(BJ.GetConvertedPlayerId(CJ.GetOwningPlayer(SU)))].Hero_ID))
...
CJ.TriggerAddAction(trig_JiNengFuWen,
    function () 
        ...
        if japi.EXGetAbilityDataString (japi.EXGetUnitAbility(SU, CJ.GetSpellAbilityId()), 1, 203)== "符文A[00"..tostring(id-1).."]" then  
            --RemoveAbility(CJ.GetTriggerUnit(),id)  
            if h.FuWenJieMian == "选择符文-技能1" then
                h["技能1符文"] = 1
                CJ.SetUnitAbilityLevel(h.UnitType, Global:GetId(HD[x] ["技能1"].ID), 2 )
            elseif h.FuWenJieMian == "选择符文-技能2" then
                h["技能2符文"] = 1
                CJ.SetUnitAbilityLevel( h.UnitType, Global:GetId(HD[x] ["技能2"].ID), 2 )
            elseif h.FuWenJieMian == "选择符文-技能3" then
                h["技能3符文"] = 1
                CJ.SetUnitAbilityLevel( h.UnitType, Global:GetId(HD[x] ["技能3"].ID), 2 )
            elseif h.FuWenJieMian == "选择符文-技能4" then
                h["技能4符文"] = 1
                CJ.SetUnitAbilityLevel( h.UnitType, Global:GetId(HD[x] ["技能4"].ID), 2 )
            elseif h.FuWenJieMian == "选择符文-技能5" then
                h["技能5符文"] = 1
                CJ.SetUnitAbilityLevel( h.UnitType, Global:GetId(HD[x] ["技能5"].ID), 2 )
            elseif h.FuWenJieMian == "选择符文-技能6" then
                h["技能6符文"] = 1
                CJ.SetUnitAbilityLevel( h.UnitType, Global:GetId(HD[x] ["技能6"].ID), 2 )
            elseif h.FuWenJieMian == "选择符文-技能7" then
                h["技能7符文"] = 1
                CJ.SetUnitAbilityLevel( h.UnitType, Global:GetId(HD[x] ["技能7"].ID), 2 )
            end
        end 
        --符文B
        if japi.EXGetAbilityDataString (japi.EXGetUnitAbility(SU, CJ.GetSpellAbilityId()), 1, 203)== "符文B[00"..tostring(id-1).."]" then  
            ...
            if h.FuWenJieMian == "选择符文-技能1" then
                h["技能1符文"] = 2
                CJ.SetUnitAbilityLevel( h.UnitType, Global:GetId(HD[x] ["技能1"].ID), 3 )
            elseif h.FuWenJieMian == "选择符文-技能2" then
                h["技能2符文"] = 2
                CJ.SetUnitAbilityLevel( h.UnitType, Global:GetId(HD[x] ["技能2"].ID), 3 )
            elseif h.FuWenJieMian == "选择符文-技能3" then
                h["技能3符文"] = 2
                CJ.SetUnitAbilityLevel( h.UnitType, Global:GetId(HD[x] ["技能3"].ID), 3 )
            elseif h.FuWenJieMian == "选择符文-技能4" then
                h["技能4符文"] = 2
                CJ.SetUnitAbilityLevel(h.UnitType, Global:GetId(HD[x] ["技能4"].ID), 3 )
            elseif h.FuWenJieMian == "选择符文-技能5" then
                h["技能5符文"] = 2
                CJ.SetUnitAbilityLevel( h.UnitType, Global:GetId(HD[x] ["技能5"].ID), 3 )
            elseif h.FuWenJieMian== "选择符文-技能6" then
                h["技能6符文"] = 2
                CJ.SetUnitAbilityLevel( h.UnitType, Global:GetId(HD[x] ["技能6"].ID), 3 )
            elseif h.FuWenJieMian == "选择符文-技能7" then
                h["技能7符文"] = 2
                CJ.SetUnitAbilityLevel( h.UnitType, Global:GetId(HD[x] ["技能7"].ID), 3 )
            end
        end 
        --符文C
        if japi.EXGetAbilityDataString (japi.EXGetUnitAbility(SU, CJ.GetSpellAbilityId()), 1, 203)== "符文C[00"..tostring(id-1).."]" then  
           ...
           if h.FuWenJieMian == "选择符文-技能1" then
            h["技能1符文"] = 3
            CJ.SetUnitAbilityLevel( h.UnitType, Global:GetId(HD[x] ["技能1"].ID), 4 )
        elseif h.FuWenJieMian == "选择符文-技能2" then
            h["技能2符文"] = 3
            CJ.SetUnitAbilityLevel( h.UnitType, Global:GetId(HD[x] ["技能2"].ID), 4 )
        elseif h.FuWenJieMian == "选择符文-技能3" then
            h["技能3符文"] = 3
            CJ.SetUnitAbilityLevel( h.UnitType, Global:GetId(HD[x] ["技能3"].ID), 4 )
        elseif h.FuWenJieMian == "选择符文-技能4" then
            h["技能4符文"] = 3
            CJ.SetUnitAbilityLevel( h.UnitType, Global:GetId(HD[x] ["技能4"].ID), 4 )
        elseif h.FuWenJieMian== "选择符文-技能5" then
            h["技能5符文"] = 3
            CJ.SetUnitAbilityLevel( h.UnitType, Global:GetId(HD[x] ["技能5"].ID), 4 )
        elseif h.FuWenJieMian == "选择符文-技能6" then
            h["技能6符文"] = 3
            CJ.SetUnitAbilityLevel( h.UnitType, Global:GetId(HD[x] ["技能6"].ID), 4 )
        elseif h.FuWenJieMian == "选择符文-技能7" then
            h["技能7符文"] = 3
            CJ.SetUnitAbilityLevel( h.UnitType, Global:GetId(HD[x] ["技能7"].ID), 4 )
        end
        end 
        --符文D
        ...
        if japi.EXGetAbilityDataString (japi.EXGetUnitAbility(SU, CJ.GetSpellAbilityId()), 1, 203)== "符文F[00"..tostring(id-1).."]" then  
            RemoveAbility(CJ.GetTriggerUnit(),id)  
            if h.JFuWenJieMian == "主界面" then
                h.FuWenJieMian = "选择符文-技能7"
        ...
        if japi.EXGetAbilityDataString (japi.EXGetUnitAbility(SU, CJ.GetSpellAbilityId()), 1, 203)== "符文G[00"..tostring(id-1).."]" then  
            RemoveAbility(CJ.GetTriggerUnit(),id)  
            if h.FuWenJieMian == "主界面" then
                h.FuWenJieMian = "选择符文-技能6"
        ...
        if japi.EXGetAbilityDataString (japi.EXGetUnitAbility(SU, CJ.GetSpellAbilityId()), 1, 203)== "符文L[00"..tostring(id-1).."]" then  
            ...
            if h.FuWenJieMian == "主界面" then
                ...
                ReturnAbility(SU,id)
            elseif h.FuWenJieMian == "选择符文-技能1" or  h.FuWenJieMian == "选择符文-技能2" or  h.FuWenJieMian == "选择符文-技能3" or  h.FuWenJieMian == "选择符文-技能4" or  h.FuWenJieMian == "选择符文-技能5" or h.FuWenJieMian == "选择符文-技能6" or  h.FuWenJieMian == "选择符文-技能7" then
                RemoveAbility(SU,id)
                IntAbility(SU,id) 
            end
