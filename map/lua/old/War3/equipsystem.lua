...
CJ.TriggerAddAction(trig1,
	function () 
		...
		if CJ.GetWidgetLife(CJ.GetManipulatedItem()) == 100 then
			...
			CJ.DisplayTextToPlayer( CJ.Player(0), 0, 0, tostring(CJ.GetUnitMoveSpeed(u))) 
			if CJ.GetUnitAbilityLevel(u, Global:GetId(j)) ~= 1 then
				...
				local xd = GM:GetHero(u):GetEquipment(x.ZiDingYi)--获取已装备的物品数据
			...
			GM:GetHero(u):SetItemprototype(x)  --添加英雄装备数据
            --替换装备说明
			local mj = GM.Maji_ID["玩家"..tostring(id)].Equipment_Maji--GM:GetHero(id).Equipment_Maji_ID
