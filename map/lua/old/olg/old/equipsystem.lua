...
CJ.TriggerAddAction(trig1,
	function () 
		...
		if CJ.GetWidgetLife(CJ.GetManipulatedItem()) == 100 then
			...
			if CJ.GetUnitAbilityLevel(u, s1:GetId(j)) ~= 1 then
				
				--设置工程升级
				CJ.UnitAddAbility( u, s1:GetId(j) )
				CJ.UnitMakeAbilityPermanent( u, true, s1:GetId(j) )
				BJ.SetPlayerAbilityAvailableBJ( false,s1:GetId(j) , CJ.GetOwningPlayer(u) )
				

			else
				local xd = GM:GetHero(id):GetZhuangBei(x.ZiDingYi)--获取已装备的物品数据
			...
			japi.EXSetAbilityDataString(japi.EXGetUnitAbility(mj, s1:GetId(mjjn)), 1, 215,s1:GetName(x))
			japi.EXSetAbilityDataString(japi.EXGetUnitAbility(mj, s1:GetId(mjjn)), 1, 218,s1:GetExplain(x))	
			japi.EXSetAbilityDataString(japi.EXGetUnitAbility(mj, s1:GetId(mjjn)), 1, 204,x.TuBiao )
