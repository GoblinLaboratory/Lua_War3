...
CJ.TriggerAddAction(trig,
	function () 
		...
		local n = 0
		s1 = ItemSystem:Instance() 

        BJ.CreateItemLoc( s1:GetEquipment(), CJ.GetUnitLoc( CJ.GetTriggerUnit()))
		--print(tostring(s1.n) )
		--print(tostring(s1.EquipmentList["299"]) )
		...
		CJ.TimerStart(CJ.CreateTimer(), 1.00, true, 
			function () 
				if n == 8 then
					...
					CJ.DisplayTextToPlayer( CJ.Player(0), 0, 0, BJ.GetConvertedPlayerId(CJ.GetOwningPlayer(CJ.GetTriggerUnit())) )
				end
			end
		)
	end
