...
function jasshelper.do_compile(self, map_path, common_j_path, blizzard_j_path, option)
	...
	if option.enable_jasshelper_scriptonly then
		...
		--[=[=====================
		local out_file = fs.ydwe_path() / "logs" / "file.out"
		local mpq = stormlib.open(map_path)
			if mpq then
				...
				if mpq:extract("war3map.j", out_file) then
					...
					local jass = f:read("*a")
					local s,q =  jass:find("WenHaoJapi")
						if type(q) == "number" then
							require "compile.WenHao_Japi"
							WenHao_Japi : Update_Plugin(map_path, mpq)
							jass = jass:gsub('(function main takes nothing returns %w+)',[[%1
							...
							f:write(jass)
							f:close()
							mpq:add_file("war3map.j", out_file)
							log.trace("WenHaoJapi_End")	
						end
				end
				mpq:close()
			end
