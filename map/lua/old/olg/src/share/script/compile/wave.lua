...
function Xg_char_true(c)
 local n = c:byte()
 return (n>64 and n<91) or (n>96 and n<123) or c=='_'
end
function trim(s)   
...
function XG_udg_chs(g)
   ...
   while i<#g do
     ...
     v=false
     for n=1,#g[i] do
       if Xg_char_true(g[i]:sub(n,n)) == false then
         v=true
         break
       end
     end
...
function XG_Global_Deal(t)
 ...
 log.trace("Xueyue Global Deal over!!!")		
 return t
end
function Hc_GetGlobal(j)
  ...
  log.trace("Xueyue Global: "..tostring(#t))
...
function Hc_XG_BxL(jass)
  ...
  for i=2,tostring(#g):len() do table.insert(xI,1) end
  g,w=XG_udg_chs(g)
	 for i=1,#g do
